...
def test_get_entrance_fee(check_local_chain, lottery_contract):
    expected_entrance_fee = Web3.toWei(0.025, "ether")
    entrance_fee = lottery_contract.getEntranceFee()
    assert expected_entrance_fee == entrance_fee


def test_cant_enter_unless_started(check_local_chain, lottery_contract):
...
def test_can_start_and_enter_lottery(check_local_chain, lottery_contract):
    account = get_account()
...
def test_can_end_lottery(check_local_chain, lottery_contract):
    account = get_account()
    lottery_contract.startLottery({"from": account})
    lottery_contract.enter(
        {"from": account, "value": lottery_contract.getEntranceFee()}
    ...
    assert lottery_contract.lottery_state() == 1


def test_can_pick_winner_correctly(check_local_chain, lottery_contract):
    account = get_account()
    lottery_contract.startLottery({"from": account})
    lottery_contract.enter(
        {"from": account, "value": lottery_contract.getEntranceFee()}
